...
<div class="pba-chat" id="pbaChat" data-pba-chat aria-modal="true" role="dialog" aria-labelledby="pbaChatTitle">
  <div class="pba-chat-header">
    ...
    <div class="pba-icon-btn" id="pbaExpandBtn"   title="Expand / Collapse">⤢</div>
    <div class="pba-icon-btn" id="pbaStageToggle" title="Show/Hide companion">✨</div>
    <div class="pba-icon-btn" id="pbaSwitcherBtn" title="Switch companion">👥</div>
    <div class="pba-icon-btn" id="pbaPromptsToggle" title="Show/Hide suggestions">💬</div>
    <div class="pba-icon-btn" id="pbaUnlockBtn" title="Unlock Pro / Premium">🔓</div>
    <div class="pba-close"    id="pbaClose"       title="Close">×</div>

    <!-- Hamburger (mobile landscape only) -->
    <div class="pba-icon-btn" id="pbaHamburger"   title="Menu">☰</div>
  </div>
  ...
  <div class="pba-chat-body" id="pbaChatBody" data-pba-messages>
    <div class="pba-msg" data-agent="astra">
      ...
      <div class="pba-msg-bubble">✨ Hi peacebuilder! I’m <strong>Astra</strong> — your Site Navigator &amp; Course Assistant.<br/>I’ll guide you to the right lessons and tools. Ask me anything to begin!</div>
    </div>
  </div>

  <!-- Composer -->
  <div class="pba-composer">
    <textarea id="pbaInput" class="pba-input" placeholder="Type a message…" data-pba-input rows="1"></textarea>
...
<div id="pbaUnlockModal" style="display:none;">
  ...
  <div class="pba-unlock-card" role="dialog" aria-modal="true" aria-labelledby="pbaUnlockTitle">
    <div class="pba-unlock-title" id="pbaUnlockTitle">🔓 Unlock Pro / Premium</div>
...
PBA.handleSend = async function(){
  ...
  try {
    ...
    if (data && Array.isArray(data.messages)) {
      ...
      PBA.uiAppend('assistant', 'Hmm, I didn’t get a usable reply. Try again?');
    }
  } catch (err) {
    console.error('PBA.handleSend error:', err);
    PBA.uiAppend('assistant', 'Connection issue—please try again.');
  } finally {
...
// ⏱️ Basic limits & UI prefs (client-side)
PBA.limits = {
  timeoutMs: 12000,
...
// 🆔 Persist a session_id (per-browser) for quotas/rate limits
PBA.session = {
  key: 'pba_session_id',
  get() {
    try {
...
(function bindUnlockUI(){
  ...
  // 🔁 IMPORTANT: refresh plan from n8n
...
if (clearBtn) {
  clearBtn.addEventListener('click', async () => {
    ...
    // 2) Clear any cached plan + quota session (THIS is what was making Pro “sticky”)
    try {
...
PBA.send = async function ({
  ...
  // 1) persisted store (pba.session.v1) → 2) window.PBA_USER_PLAN → 3) passed tier → 4) lite
...
const payload = {
  ...
  tier: planNormalized,     // ✅ FIX: force payload to match persisted plan
...
/* ==============================
   PBA::UI helpers  (v2 — align with .pba-msg-bubble DOM)
   ============================== */
...
PBA.uiTyping = function(show) {
  if (show) {
    ...
    const t = PBA.uiAppend('assistant', 'typing…');
...
(() => {
  try {
    ...
    `;
    panel.innerText = 'PBA Debug — hidden.\n(Long-press “Astra” title to toggle.)';
    ...
    window.fetch = async (input, init = {}) => {
      ...
      if (bodyPreview) logLine(`→ Body: ${bodyPreview}`);
      try {
        const res = await nativeFetch(input, { ...init, signal: ctrl.signal });
        clearTimeout(to);
        logLine(`← STATUS ${res.status} (${res.type || 'basic'}) for ${url}`);
        ...
        try {
          ...
          if (text && text.length > 0) {
            logLine(`← Body (${Math.min(text.length,400)} chars): ${text.slice(0,400)}${text.length>400?'…(+)':''}`);
          }
        } catch {
          /* ignore */
        }
        ...
        if (res.status === 0) logLine('! Network error (status 0) — possible mixed content or CORS.');
        if (res.status === 401 || res.status === 403) logLine('! Auth/Origin blocked — check n8n origin/headers allowlist.');
        if (res.status === 413) logLine('! Payload too large — reduce body or raise n8n limit.');
        if (res.status === 429) logLine('! Rate limited.');
        if (res.status >= 500) logLine('! Server error (5xx) — check n8n logs / Cloudflare / node timeouts.');

        return res;
      } catch (err) {
        clearTimeout(to);
        logLine(`× FETCH ERROR: ${String(err && err.message || err)}`);
...
if (stage){
  stage.addEventListener('click', (ev)=>{
    ...
    const token = drawAnimToken();     // ← pull one from the 6-token bag
    runUniversalAddOn(ctx, token);     // ← run that effect
  });
}
...
function sparkleBurst(container, x, y, count=12){
  ...
  for(let i=0;i<count;i++){
    ...
    // center particle (14px → half size = 7px offset)
    s.style.left = (cx - 7) + 'px';
    s.style.top  = (cy - 7) + 'px';

    // wider spread: 60–120px at random angle
...
function triggerSparkle(el, wrap, x, y){
  ...
  // 4 bursts: center → left → right → top (staggered, more spaced)
  const bursts = [
...
function runUniversalAddOn(ctx, token){
  ...
  break;
    ...
    const removeBackdrop=()=>{
      ...
      const b=document.getElementById('pbaExpandBtn'); if(b) b.textContent='⤢';
    };
    ...
    const introCopy = {
      lumina:  "Hi! I’m Lumina — your Inner Peace Guide. I help you with self-development and purpose-alignment.",
      paxion:  "Hello! I’m Paxion — your Peacebuilder Strategist. I help you master negotiation and mediation to turn conflict into collaboration.",
      impaxis: "Hey! I’m Impaxis — your Impact Venture Builder. I help you turn ideas into impact and income."
    };
...
if(stageImg){
  ...
  setTimeout(()=>{
    ...
    /* ===== Hamburger (mobile landscape) — minimal menu; known limitation ===== */
    if(hamburger){
      let menuEl=null;
      const buildMenu=()=>{
...
(function bindComposer(){
  function init(){
    ...
    // click send — reset listeners by cloning
...
/* ===== END Composer (Aura-aware, DOM-ready) ===== */
    ...
    /* ===== Message helpers — per-message avatar lock ===== */
...
PBA.handleSend = async function() {
  ...
  if (!res || !res.ok) {
    PBA.uiAppend('assistant', 'Sorry—something went wrong. Please try again.');
    return;
  }
...
<!-- ===== PBA LITE QUOTA — SLICE 1 (Astra-themed heads-up after 7th reply) ===== -->
<script>
(function(){
  ...
  const TEMPLATE_FIRST      = false;  // we’ll keep cloning for now; template may come later
  ...
  const HEADS_UP_TEXT = '✨ You’re making great progress, peacebuilder! Just a heads-up — 3 free messages remain in your Lite session today.';

  // (n9 mini-counter disabled — keep n7 heads-up + n10 cap only)
  const MINI_COUNTER_AT   = 999;    // never reached in Lite (10/day)
  const MINI_COUNTER_TEXT = '';     // no mini-counter text
  const CAP_AT            = 10;
  const UPSELL_TEXT       = '✨ (10/10 free daily messages used)<br/>' + '💫 Resolve conflict. Launch your impact business. Earn a living doing good.<br/><br/>' +
    'Pro gives you everything you need to learn faster, build smarter, and grow sustainably — with expert support, guided tools, and AI-powered peacebuilder companions by your side.<br/><br/>' +
    '🔓 Upgrade to Pro and start turning your mission into momentum.'; // later: swap in your upgrade URL
  ...
  // new day → reset BOTH top-level and nested sess
  if (s.dateKey !== today){
    const newSess = {
    ...
    console.log('[PBA] daily reset →', today);
  }
}
...
function enableInputAfterReset(){
  ...
  console.log('[PBA] daily reset → composer unlocked');
}
